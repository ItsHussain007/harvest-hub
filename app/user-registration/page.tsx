...
const RegistrationPage: React.FC = () => {
  ...
  const handleSubmit = () => {
    if (validateStep(4)) {
      ...
      // In a real app, redirect or show success state here
    }
  };
          ...
          <div className="space-y-4 w-full">
            ...
            <div className="pt-4 text-center">
              <p className="text-sm text-gray-500">
                ...
                <a href="/login" className="text-green-600 font-medium">Log in</a>
              </p>
            </div>
          </div>
        );
          ...
          return (
            <div className="text-center py-6">
              ...
              <p className="text-gray-600">
                You've successfully created your account with {loginMethod} authentication.
              </p>
              <button
                onClick={() => {
                  handleSubmit();
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {[
                    ...
                    <div key={index} className="flex items-center gap-2">
                      <div
                        className={`w-4 h-4 rounded-full flex items-center justify-center ${req.met ? 'bg-green-600' : 'bg-gray-200'}`}
                      >
  ...
  const renderMobileProgress = () => {
    ...
    const totalSteps = loginMethod === 'email' ? 4 : 2;
  ...
  const renderDesktopProgress = () => {
    if (step === 0) return null;
    const labels = loginMethod === 'email' ? ['Account Type', 'Email', 'Personal Info', 'Password'] : ['Account Type', 'Complete'];
      ...
      <div className="hidden md:flex justify-between mb-8 relative">
        {steps.map((number, index) => (
          <div key={number} className="flex-1 text-center">
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center mx-auto mb-2 ${step >= number ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-400'}`}
            >
  ...
  const getHeaderText = () => {
    if (step === 0) return 'Sign up to continue';
    if (step === 1) return 'Choose Account Type';
    if (step === 2) return "What's your email?";
    if (step === 3) return 'Tell us about yourself';
    if (step === 4) return loginMethod === 'email' ? 'Create a password' : 'All set!';
    return '';
  };

  const getSubHeaderText = () => {
    if (step === 0) return 'Select how you want to sign up';
    if (step === 1) return 'Select the type of account you want to create';
    if (step === 2) return "We'll use this to keep your account secure";
    if (step === 3) return 'Help us personalize your experience';
    if (step === 4) return loginMethod === 'email' ? "Make sure it's secure" : 'Your account is ready to use';
    return '';
  };
              ...
              <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4">
                {step > 0 && (
                  <button
                    ...
                    className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-4 rounded-xl shadow-sm transition-colors"
                  >
                    {step === 4 && loginMethod === 'email'
                      ? 'Create Account'
                      : step === 4 && loginMethod !== 'email'
